...
# Usage: ${CLAUDE_PLUGIN_ROOT}/scripts/status/ai-status.sh [options]

set -e

# Source shared libraries
# Use plugin root if available (when run as plugin), otherwise use script directory
if [[ -n "${CLAUDE_PLUGIN_ROOT:-}" ]]; then
    # Running as plugin
    LIB_DIR="${CLAUDE_PLUGIN_ROOT}/scripts/lib"
